...
- `apps/web/**` / `apps/admin/**` / `content/posts/**` / 根 `package.json` 等改动 → `deploy-pages.yml` 自动触发，上 GitHub Pages + Cloudflare Pages + Lighthouse + IndexNow/Baidu/Google ping。
- 后端代码改动不会自动部署，要 `pnpm deploy:server` 手动 rsync + docker。

## CI / 质量门

- **`.github/workflows/ci.yml`** 在 PR 和 push 到 main 时跑两个并发 job：
  - `Frontend (typecheck + build)`：`pnpm install` → `pnpm --filter @tenggouwa/web build` 和 `@tenggouwa/admin build`（两个 build 命令本身就是 `tsc --noEmit && vite build`）。
  - `Backend (ruff + pytest)`：`uv sync --extra dev` → `ruff check .` + `ruff format --check .` + 条件性 `pytest`（当前 `apps/server/tests` 为空，pytest skip）。
