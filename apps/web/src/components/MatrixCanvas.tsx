...
// 矩阵雨 canvas，尺寸跟随容器（offsetWidth/Height）。lab/matrix 页面与 Konami 彩蛋共用。
export default function MatrixCanvas({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function step() {
      ...
      for (let i = 0; i < cols; i++) {
        ...
        ctx.fillText(GLYPHS[Math.floor(Math.random() * GLYPHS.length)], x, y - fontSize * dpr);
  ...
  return <canvas ref={ref} className={className ?? 'block h-full w-full bg-terminal-bg'} />;
}
