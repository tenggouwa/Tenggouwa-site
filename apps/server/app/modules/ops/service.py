...
_LIVE_SMOKE_URL = (
    "https://api.github.com/repos/tenggouwa/Tenggouwa-site/actions/workflows/live-smoke.yml/runs?per_page=1"
)
_LIVE_SMOKE_CACHE_SECONDS = 60
_live_smoke_cache: tuple[float, OpsLiveSmoke] | None = None


class OpsService:
    async def _latest_live_smoke(self) -> OpsLiveSmoke:
        ...
        try:
            ...
            if not run:
                value = OpsLiveSmoke(status="unknown", summary="暂未找到夜间冒烟记录。")
            else:
            ...
            value = OpsLiveSmoke(status="unknown", summary="暂时无法读取夜间冒烟状态。")
    ...
    async def overview(self, session: AsyncSession) -> OpsOverview:
        ...
        return OpsOverview(
            ...
            live_smoke=await self._latest_live_smoke(),
        )
