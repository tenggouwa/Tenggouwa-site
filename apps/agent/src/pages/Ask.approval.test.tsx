...
describe('Ask approval flow', () => {
  it('approval 事件渲染审批卡；批准后 resume 请求带 { approvals, session_id }', async () => {
    ...
    expect(bodies[0]).toEqual({ q: '删掉 x', session_id: null, auto_approve: false, deep_think: false, reflect: false, auto_model: false });
    expect(bodies[1]).toEqual({ approvals: { c1: true }, session_id: 's1', auto_approve: false, deep_think: false, reflect: false, auto_model: false });
  ...
  it('多步：续跑再抛 approval → 弹全新可操作的审批卡（不被上轮 submitted 锁死）', async () => {
    ...
    expect(bodies[1]).toEqual({ approvals: { c1: true }, session_id: 's1', auto_approve: false, deep_think: false, reflect: false, auto_model: false });
    expect(bodies[2]).toEqual({ approvals: { c2: true }, session_id: 's1', auto_approve: false, deep_think: false, reflect: false, auto_model: false });
  });
});
